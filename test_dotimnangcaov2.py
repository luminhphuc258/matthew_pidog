...
# IMPORTANT: do not reject near-top markers; mark for debug only
...
# =========================
# Local X detection + overlay
# =========================

def _cell_bbox(h: int, w: int, rows: int, cols: int, r: int, c: int) -> Tuple[int, int, int, int]:
...
def detect_x_in_cell(cell_bgr: np.ndarray) -> bool:
...
def detect_x_cells(warp_bgr: np.ndarray, rows: int, cols: int) -> set:
    ...
    for r in range(rows):
        for c in range(cols):
            x0, y0, x1, y1 = _cell_bbox(h, w, rows, cols, r, c)
            margin = int(round(min(x1 - x0, y1 - y0) * X_ROI_MARGIN))
            rx0 = clamp(x0 + margin, 0, w - 1)
            ry0 = clamp(y0 + margin, 0, h - 1)
            rx1 = clamp(x1 - margin, rx0 + 1, w)
            ry1 = clamp(y1 - margin, ry0 + 1, h)
            roi = warp_bgr[ry0:ry1, rx0:rx1]
            if detect_x_in_cell(roi):
                x_cells.add((r, c))
    return x_cells


def draw_grid_overlay(base_bgr: np.ndarray, rows: int, cols: int, x_cells: set) -> np.ndarray:
    ...
    out = cv2.addWeighted(overlay, OVERLAY_ALPHA, base_bgr, 1.0 - OVERLAY_ALPHA, 0)
    ...
    for r in range(rows):
        for c in range(cols):
            x0, y0, x1, y1 = _cell_bbox(h, w, rows, cols, r, c)
            cv2.putText(out, f"{r},{c}", (x0 + 4, y0 + 16),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)

    return out

# =========================
# Marker-only Detector (FIX: do not reject top markers)
# =========================
class MarkerBoardDetector:
    ...
    def _extract_candidates(self, frame_bgr) -> Dict[str, Any]:
        ...
        for c in contours:
            ...
            # OK: do not reject; only color + label for debug
            if near_top:
    ...
    def _pick_slots(self, candidates: List[Dict[str, Any]], w: int, h: int) -> Dict[str, Any]:
        ...
        # fallback if only top markers found
        if "tl" not in slots:
...
<head>
  <meta charset="utf-8"/>
  <title>Scan Board 6x4 (opencv local)</title>
  <style>
...
<body>
  <div class="wrap">
    <div class="left">
      <div class="card">
        ...
        <div class="kv">
          ...
          <div class="muted" style="margin-top:8px;">NEW: warp -> overlay grid + detect X locally.</div>
        </div>
    ...
    <div class="stages">
      ...
      <div class="muted">Always show candidates/picked/warp/grid_clean/overlay.</div>
...
# =========================
def main():
    print("[START] marker_only_board_bbox_opencv_local", flush=True)
    ...
    while True:
        ...
        try:
            ...
            if det.get("found") and det.get("bbox") is not None:
                ...
                # keep warp for local overlay
            else:
                stages["5_warp"] = _make_black_warp("NO_WARP (need >=3 markers)")

            # Local overlay + X detection (no server)
            overlay_base = warp
            if overlay_base.shape[:2] != (WARP_H, WARP_W):
                overlay_base = cv2.resize(overlay_base, (WARP_W, WARP_H))

            x_cells = detect_x_cells(overlay_base, GRID_ROWS, GRID_COLS)
            overlay = draw_grid_overlay(overlay_base, GRID_ROWS, GRID_COLS, x_cells)
            stages["6_grid_overlay"] = overlay

            cells = [{"row": r, "col": c, "state": "player_x"} for (r, c) in sorted(x_cells)]
            cam.set_cells_server(cells)
            board_mat = [[1 if (r, c) in x_cells else 0 for c in range(GRID_COLS)] for r in range(GRID_ROWS)]
            board.set_board(board_mat)

            cam.set_stage_images(stages)
            cam.set_scan_status("ready", "")
            cam.log(f"[SCAN] ok: x_cells={len(x_cells)} bbox={det['bbox']}")
