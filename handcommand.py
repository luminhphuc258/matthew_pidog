...
class HandCfg:
    ...
    # palm position for stand/sit
    hand_high_y: float = 0.33
    hand_low_y: float = 0.70

    # thumb direction
    thumb_dir_deadzone: float = 0.10

    # ---- NEW: finger detect thresholds ----
    # ratio: tip->wrist / mcp->wrist, small => finger curled
    ext_ratio_threshold: float = 1.08   # > => likely extended
    curl_ratio_threshold: float = 0.95  # < => likely curled

    # ---- NEW: fist logic ----
    fist_curl_min: float = 0.72         # avg curl score
    fist_hold_to_stop_sec: float = 0.70 # hold fist this long => STOP after BACK

    # ---- NEW: wave detect ----
    wave_window_sec: float = 0.85       # store palm_x history window
    wave_min_amplitude: float = 0.12    # normalized x amplitude required
    wave_min_swings: int = 2            # left-right-left (2 sign changes) within window
...
class HandCommand:
    ...
    # gesture names
    G_NONE = "NONE"
    G_WAVE = "WAVE"              # vẫy tay -> trot
    G_BECKON = "BECKON"          # beckon / come here -> forward (optional)
    G_INDEX_UP = "INDEX_UP"
    ...
    G_OPEN_PALM = "OPEN_PALM"    # 4-5 ngón duỗi (không vẫy)
    ...
    def __init__(
        ...
        # NEW: wave history [(ts, palm_x)]
        self._palm_hist: List[Tuple[float, float]] = []
    ...
    def draw_on_frame(self, frame_bgr):
        ...
        Draw:
          - landmarks
          - highlight EXTENDED fingers in green (tips + bones)
        ...
        try:
            ...
            if pts is not None:
                ...
                # which fingers extended?
                fingers = {
                    "thumb": ("thumb", [2, 3, 4]),
                    "index": ("index", [IN_MCP, IN_PIP, IN_DIP, IN_TIP]),
                    "middle": ("middle", [MD_MCP, MD_PIP, MD_DIP, MD_TIP]),
                    "ring": ("ring", [RG_MCP, RG_PIP, RG_DIP, RG_TIP]),
                    "pinky": ("pinky", [PK_MCP, PK_PIP, PK_DIP, PK_TIP]),
                }

                for name, (_, chain) in fingers.items():
                    ...
                    # draw tip circle
                    tip_i = chain[-1]
                    cv2.circle(frame_bgr, P(tip_i), self.cfg.draw_tip_radius, col, -1)

                # also highlight thumb tip
                col_th = (0, 255, 0) if bool(fext.get("thumb", False)) else (80, 80, 80)
                cv2.circle(frame_bgr, P(TH_TIP), self.cfg.draw_tip_radius, col_th, -1)
    ...
    def _curl_score(self, pts, tip, mcp, wrist=0) -> float:
        ...
        # map ratio to 0..1
        # ratio small -> curled => score high
        # ratio big  -> extended => score low
        return _clamp((1.05 - r) / 0.35, 0.0, 1.0)

    def _thumb_extended(self, pts, handedness_label: Optional[str]) -> bool:
        # thumb is tricky. We do simple:
        # compare thumb tip distance to index mcp and wrist.
        WRIST = 0
        TH_TIP = 4
        IN_MCP = 5
        d_tw = math.sqrt(self._dist2_xy(pts, TH_TIP, WRIST))
        d_ti = math.sqrt(self._dist2_xy(pts, TH_TIP, IN_MCP))
        # if thumb tip far from palm and also far from index base => likely extended
        return (d_tw > 0.18) and (d_ti > 0.10)

    def _detect_wave(self, palm_x: float) -> bool:
        # keep history in window
        now = _now()
        self._palm_hist.append((now, palm_x))
        cut = now - self.cfg.wave_window_sec
        while self._palm_hist and self._palm_hist[0][0] < cut:
            self._palm_hist.pop(0)

        if len(self._palm_hist) < 6:
            return False

        xs = [x for _, x in self._palm_hist]
        amp = (max(xs) - min(xs))
        if amp < self.cfg.wave_min_amplitude:
            return False
    ...
    def _infer_gesture(self, frame_bgr) -> Tuple[str, Dict[str, Any]]:
        ...
        if not res.multi_hand_landmarks:
            self._last_mp_landmarks = None
            self._last_pts = None
            # reset wave/fist trackers softly
            self._palm_hist.clear()
            self._fist_start_ts = None
            self._fist_back_fired = False
            self._fist_stop_fired = False
        ...
        # handedness label (Right/Left) if available
        handed_label = None
        try:
            handed_label = (res.multi_handedness[0].classification[0].label or "").strip()
        except Exception:
            handed_label = None
        ...
        TH_MCP = 2
        TH_TIP = 4
        IN_MCP, IN_PIP, IN_TIP = 5, 6, 8
        MD_MCP, MD_PIP, MD_TIP = 9, 10, 12
        RG_MCP, RG_PIP, RG_TIP = 13, 14, 16
        PK_MCP, PK_PIP, PK_TIP = 17, 18, 20
        ...
        if palm_y < self.cfg.hand_high_y:
            return self.G_PALM_HIGH, {"palm_xy": (palm_x, palm_y)}
        if palm_y > self.cfg.hand_low_y:
            return self.G_PALM_LOW, {"palm_xy": (palm_x, palm_y)}
        ...
        th_ext = self._thumb_extended(pts, handed_label)
        ...
        # -------- FIST (nắm tay) --------
        # điều kiện: 4 ngón cuộn nhiều và finger_count thấp
        ...
        if abs(th_dx) > abs(th_dy) and abs(th_dx) > self.cfg.thumb_dir_deadzone:
            if th_dx > 0:
                return self.G_THUMB_RIGHT, info
            else:
                return self.G_THUMB_LEFT, info

        # -------- OPEN PALM / WAVE --------
        # nếu 4+ ngón mở => có thể là open palm hoặc wave
        if finger_count >= 4:
            if self._detect_wave(palm_x):
                return self.G_WAVE, info
            return self.G_OPEN_PALM, info

        # -------- BECKON (optional) --------
        # beckon: index/middle/ring/pinky không duỗi hết nhưng cũng không fist chặt
    ...
    # -------------------------
    # Action mapping + safety + fist hold logic
    # -------------------------

    def _maybe_fire_action(self, gesture: str):
        now = _now()

        # special: fist hold logic
        if gesture == self.G_FIST:
            ...
            # if keep holding fist => STOP once
    ...
    def _map_gesture_to_action(self, gesture: str, robot_state: str) -> Tuple[Optional[str], Optional[str], bool, str]:
        ...
        # fist handled by hold logic in _maybe_fire_action()
        if gesture == self.G_FIST:
        ...
        # open palm: mình map STOP cho an toàn (bạn thích gì đổi ở đây)
